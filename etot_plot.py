...
data = pd.read_csv('_data/mos2_ecut.csv', 
                    sep=',',skip_blank_lines=True,header=0,names=["Energy Cutoff", "Total Energy","Wall Time"])
...
fig = sb.scatterplot(data=data, x="Energy Cutoff", y=total_energy).figure
#data.plot.scatter(x = 3, y = 1, figsize=(20,10),s=300)
plt.gca().yaxis.set_major_formatter(mtick.FormatStrFormatter('%.6f'))

plt.title('Total Energy vs Energy Total')
plt.tight_layout()

plt.show()

fig.savefig("etot_vs_ecut.png")
